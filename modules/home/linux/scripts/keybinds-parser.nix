# AWK-based parser that extracts keybindings from modules/home/hyprland/binds.nix
# and outputs them in a structured format for rofi and QuickShell display tools.
{pkgs}:
pkgs.writeShellScriptBin "keybinds-parser" ''
    ...
    case "$MODE" in
      hyprland)
        # First, try to parse /etc/nixos bindd entries from Nix config for richer descriptions
  ...
  BEGIN {
      ...
      sway)
        ...
        if [[ ! -f "$SWAY_CONFIG" ]]; then
          SWAY_CONFIG="$HOME//etc/nixos/modules/home/gui/sway/files/config_cuerdos"
          if [[ ! -f "$SWAY_CONFIG" ]]; then
