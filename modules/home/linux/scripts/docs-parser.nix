# CLI tool that indexes and retrieves documentation markdown files from /etc/nixos/docs.
# Same structure as cheatsheets-parser; supports category/language listing and content fetch.
{pkgs}:
pkgs.writeShellScriptBin "docs-parser" ''
  ...
  MODE: files|content
  CATEGORY: category name matching docs/ subdirectories
  LANGUAGE: en|es
