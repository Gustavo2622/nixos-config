...
in {
  programs.kitty = {
    ...
    extraConfig = ''
      # Symbol fallback: map Unicode blocks absent from the primary font to the fallback.
      # Kitty resolves glyphs via its own engine, not fontconfig family-append rules,
      # so explicit symbol_map is required for reliable coverage.
      symbol_map U+2190-U+21FF ${vars.fallbackFont}
      symbol_map U+27F0-U+27FF ${vars.fallbackFont}
      symbol_map U+2900-U+297F ${vars.fallbackFont}
      symbol_map U+2200-U+22FF ${vars.fallbackFont}
      symbol_map U+2500-U+259F ${vars.fallbackFont}
      symbol_map U+2460-U+24FF ${vars.fallbackFont}
