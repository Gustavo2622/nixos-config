# CLI tool that indexes and retrieves markdown cheatsheets from /etc/nixos/cheatsheets.
# Supports list-by-category, full-content fetch, and language filtering; uses find, grep, jq, sed.
{pkgs}:
pkgs.writeShellScriptBin "cheatsheets-parser" ''
    #!/usr/bin/env bash
    set -euo pipefail

    # Default paths (ported to NixOS config location)
    CHEATSHEETS_DIR="/etc/nixos/cheatsheets"
