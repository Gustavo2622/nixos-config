# QuickShell/QML logout menu with 6 actions (lock, logout, suspend, hibernate, shutdown,
# reboot); icons bundled via Nix store; supports -es flag for Spanish labels.
{
  pkgs,
  inputs ? null,
}: let
