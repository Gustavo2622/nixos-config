# QuickShell/QML cheatsheet viewer with category and language switching, real-time search
# with match highlighting, pagination, and Pandoc markdown-to-HTML rendering with caching.
{
  config,
  pkgs,
}:
pkgs.writeShellScriptBin "qs-cheatsheets" ''
  ...
  EOF
    ...
    if [[ "$CATEGORY" != "root" && ! -d "/etc/nixos/cheatsheets/$CATEGORY" ]]; then
      echo "Error: Category directory '$CATEGORY' not found in cheatsheets/" >&2
      echo "Available categories:" >&2
      echo "  root" >&2
      ls -1 "/etc/nixos/cheatsheets/" | grep -E '^[a-z]' | head -10 >&2
      exit 1
    fi
    ...
    if [ -d "/etc/nixos/cheatsheets" ]; then
      for category_dir in "/etc/nixos/cheatsheets"/*/; do
  ...
  Window {
    ...
    function loadFileContent(filename) {
      ...
      if (selectedCategory === "root") {
        filePath = "/etc/nixos/cheatsheets/" + filename;
      } else {
        filePath = "/etc/nixos/cheatsheets/" + selectedCategory + "/" + filename;
      }
