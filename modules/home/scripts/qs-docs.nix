# QuickShell/QML documentation viewer for /etc/nixos/docs.
# Same architecture as qs-cheatsheets: category switching, search, HTML rendering via Pandoc.
{pkgs}:
pkgs.writeShellScriptBin "qs-docs" ''
  ...
  Options:
    -c CATEGORY   Category to display (docs subdirectory names) (default: AI)
  ...
  EOF
    ...
    if [[ "$CATEGORY" != "root" && ! -d "/etc/nixos/docs/$CATEGORY" ]]; then
      echo "Error: Category directory '$CATEGORY' not found in docs/" >&2
      echo "Available categories:" >&2
      echo "  root" >&2
      ls -1 "/etc/nixos/docs/" | grep -E '^[a-zA-Z]' | head -10 >&2
      exit 1
    fi
    ...
    if [ -d "/etc/nixos/docs" ]; then
      for category_dir in "/etc/nixos/docs"/*/; do
  ...
  Window {
    ...
    function loadFileContent(filename) {
      ...
      if (selectedCategory === "root") {
        filePath = "/etc/nixos/docs/" + filename;
      } else {
        filePath = "/etc/nixos/docs/" + selectedCategory + "/" + filename;
      }
