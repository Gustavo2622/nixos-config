# Welcome to NixOS! A Beginner's Guide to Customization

Welcome! This guide is for users who are new to Nix and want to make some common customizations to their NixOS setup. We'll keep it simple and focus on the essentials.
...
-   `hosts/`: This directory contains the configuration for each individual computer (or "host") you've installed NixOS on.
...
-   `modules/`: This directory contains the bulk of the configuration, broken down into smaller, reusable pieces ("modules").
    -   `modules/core/global-packages.nix`: You can add packages here if you want them to be installed on *all* of your NixOS computers.
...
If you want a package to be installed on every computer you use NixOS on, you'll add it to `modules/core/global-packages.nix`. The process is the same as above.
